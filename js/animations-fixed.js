/**
 * GSAP Webflow Animation System for Nina Blaut Portfolio - SWIPER COMPATIBLE
 * Fixed version that preserves Swiper.js functionality
 * Addresses conflicts between GSAP and Swiper transforms
 */

// Console identification
console.log('Nina Blaut GSAP Animations (Swiper Compatible): Script loaded');
...
const utils = {
  ...
  detectElements() {
    ...
    return {
      ...
      // Image and slider elements (SWIPER-SAFE)
      media: {
  ...
  // SWIPER PROTECTION: Check if element is part of Swiper
...
class ProjectAnimations {
  ...
  init() {
    ...
    console.log('GSAP Animations: Initializing project animations (Swiper-safe)');
  ...
  setupProjectReveal() {
    // SWIPER-SAFE: Only animate left sections, not swiper containers
    gsap.utils.toArray(this.elements.projectFlex).forEach((project, index) => {
      ...
      // Only animate left section - leave Swiper containers alone
...
class MediaAnimations {
  ...
  init() {
    ...
    console.log('GSAP Animations: Initializing media animations (Swiper-safe)');
    this.setupImageReveal();
    this.setupSwiperSafeAnimations();
  ...
  setupImageReveal() {
    // SWIPER-SAFE: Only animate images that are NOT in swiper slides initially
    gsap.utils.toArray(this.elements.img).forEach(img => {
      // Skip images inside Swiper - let Swiper handle them
      if (utils.isSwiperElement(img)) return;
  ...
  setupSwiperSafeAnimations() {
    // SWIPER-SAFE: Only add subtle hover effects to cards, not transforms
    gsap.utils.toArray(this.elements.swiperSlide).forEach(slide => {
      const card = slide.querySelector(this.elements.card);
      if (!card) return;

      // Use CSS custom properties instead of transforms to avoid conflicts
...
// Main Animation Controller - SWIPER COMPATIBLE
class NinaBlautAnimations {
  ...
  async init() {
    ...
    console.log('GSAP Animations: Initializing Nina Blaut animation system (Swiper compatible)');
    
    // CRITICAL: Wait for Swiper to be initialized first
    ...
    console.log('GSAP Animations: System initialized successfully (Swiper preserved)');
  }
  ...
  // CRITICAL: Wait for Swiper to be initialized
  async waitForSwiper() {
    console.log('GSAP Animations: Waiting for Swiper initialization...');
    ...
    while (maxAttempts > 0) {
      ...
      if (swiperElements.length > 0) {
        // Check if at least one swiper is initialized
        let initialized = false;
        swiperElements.forEach(el => {
          if (el.swiper) initialized = true;
        });
        if (initialized) {
          console.log('GSAP Animations: Swiper detected and initialized');
          break;
        }
      }
      await new Promise(resolve => setTimeout(resolve, 100));
...
// SWIPER-COMPATIBLE INITIALIZATION
let ninaAnimations;

// Wait for GSAP and Swiper to be available
function initWhenReady() {
  if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
    ninaAnimations = new NinaBlautAnimations();
    // Delay initialization to ensure Swiper goes first
    setTimeout(() => {
      ninaAnimations.init();
    }, 500);
  } else {
...
console.log('Nina Blaut GSAP Animations (Swiper Compatible): Script setup complete'); 